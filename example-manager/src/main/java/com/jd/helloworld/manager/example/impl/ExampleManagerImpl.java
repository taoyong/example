...
public class ExampleManagerImpl extends BaseManager implements ExampleManager {
    ...
    public PaginatedList<Example> findExamples(String name,int pageIndex, int pageSize) {
        ...
        baseQuery.setStartRow(getMySqlStartRow(examples.getStartRow()));//设置数据库的起始位置
    ...
    public PaginatedList<Example> findExamplesByTime1(String dateStart, String dateEnd, int pageIndex, int pageSize) {
        ...
        if(StringUtils.isNotBlank(dateStart)) { //设置查询参数
            baseQuery.setDateStart(dateStart); //一般情况下不让使用like
        }
        if(StringUtils.isNotBlank(dateEnd)) { //设置查询参数
            baseQuery.setDateEnd(dateEnd); //一般情况下不让使用like
        }

        int totalItem = exampleDao.getExamplesCount1(baseQuery); //得到总的记录数量
        ...
        List<Example> exampleList = exampleDao.findExamplesPage1(baseQuery);//查询得到相应的记录
